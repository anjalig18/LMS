...
const CourseCard = ({course}) => {
  const {currrency , calculateRating} = useContext(AppContext);
  return (
    <Link to={'/course/' + course._id} onClick={()=> scrollTo(0,0)} className='border border-gray-500/30 pb-6 overflow-hidden rounded-lg'>
      ...
      <div className='p-3 text-left'>
        ...
        <p className='text-base font-semibold text-gray-800'>{currrency}{(course.coursePrice - course.discount * course.coursePrice /100).toFixed(2)}</p>
      </div>
      </Link>
  )
}
